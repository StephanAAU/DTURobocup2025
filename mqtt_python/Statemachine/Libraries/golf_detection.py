...
class GolfDetector:
    def __init__(self, camera_matrix, dist_coeffs, ball_diameter_mm=43):
        ...
        # Defualt
        self.ORANGE_LOWER = np.array([0, 75, 93])
    ...
    def create_orange_mask(self, hsv):
        """Create a mask for detecting blue balls."""
        return cv.inRange(hsv, self.ORANGE_LOWER, self.ORANGE_UPPER)
    ...
    def process_frame(self, frame):
        ...
        mask_orange = self.create_orange_mask(hsv)

        # Cluster the red and blue balls

        orange_clusters = self.cluster_balls(frame, mask_orange, "orange")
        ...
        for cluster in orange_clusters:
...
golf_detector = GolfDetector(camera_matrix, distortion_matrix)
