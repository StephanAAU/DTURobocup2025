...
if __name__ == "__main__":
    ...
    image = ball_detector.process_frame(image)

    Alive = True
    while Alive:
        print("Ball Detection")
        for id, coords in zip(ball_detector.ball_color, ball_detector.ball_coordiantes):
            print(f"Ball: {id} at = ({coords[0]:.3f}, {coords[1]:.3f}, {coords[2]:.3f})")
            print(f"Alt coords: {ball_detector.ball_alt_coordinates}")
